...
def live_engine_setup(param_file: str, device_overwrite: str = None, debug: bool = False,
    ...
    while not converges and n < n_max:
        ...
        for i in range(first_epoch, param.HyperParameter.epochs):
            ...
            save_file = False # saving the data of our simulations

            if param.Simulation.mode in 'acquisition':
                ds_train.sample(True)
            elif param.Simulation.mode in 'static_simulation':
                ds_train.sample(True)   
                # print("ds_train.emitter type: ", type(ds_train.emitter))
                # print(ds_train.emitter) 
                if save_file == True:
                    print("saved simulation ", i)
                    for (j, v) in enumerate(ds_train.emitter):
                        train_save_name = "Data_simulation/training_simu_" + str(i) + "_frame_" + str(j) + ".h5"
                        v.save(train_save_name)
...
def setup_trainer(simulator_train, simulator_test, logger, model_out, ckpt_path, device, param):
    ...
    if param.Simulation.mode == 'acquisition':
        ...
        print("samples mode in live_engine")
    ...
    elif param.Simulation.mode == 'static_simulation':
        print("SMLMStaticDataset_simulations dataset in live_engine")

        # Training
        train_ds = decode.neuralfitter.dataset.SMLMStaticDataset_simulations(
            # simulator=simulator_train,
            frames = None,
            emitter = None, # MB: necessary
            em_proc=em_filter,
            frame_proc=frame_proc,
            bg_frame_proc=None, #bg_frame_proc, 1.10.2021
            tar_gen=tar_gen,
            weight_gen=None,
            frame_window=param.HyperParameter.channels_in,
            return_em=False)
            # ds_len=param.HyperParameter.pseudo_ds_size)
        train_ds.sample(True, split = True)

        # Test
        test_ds = decode.neuralfitter.dataset.SMLMStaticDataset_simulations(
            # simulator=simulator_train,
            frames = None,
            emitter = None, # MB: necessary
            em_proc=em_filter,
            frame_proc=frame_proc,
            bg_frame_proc=None, #bg_frame_proc
            tar_gen=tar_gen,
            weight_gen=None,
            frame_window=param.HyperParameter.channels_in,
            return_em=False)

        test_ds.sample(True, split = False)
